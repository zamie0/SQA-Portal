...
function HelpLayout() {
  ...
  // Show landing only for /help exactly. Child routes render through Outlet (their own Shell).
  if (path === "/help") {
    return (
      <RequireAuth>
        <Shell>
          ...
          <section className="grid md:grid-cols-3 gap-4">
            {cards.map((c) => (
              <Link key={c.to} to={c.to} className="rounded-3xl glass glass-hover p-6 block group">
                <div
